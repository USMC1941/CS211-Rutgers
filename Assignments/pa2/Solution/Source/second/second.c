		...
		while (r != EOF) {
			if (r == 2) {
				...
				if (fp_dict == NULL || fp_data == NULL) {
					...
					//
					increaseReversePrefix(ppRoot, 0);
					//
					i++;
					//
					char str[64];
...
void matchStr(char* str) {
	...
	for (i=0; i < (len - 1); i++) {
		...
		if (isalpha(oneChar)) {
			...
			//
			if (ppNodes) {
				ppNodes = checkLetterForWordAndReversePrefix(ppNodes, oneChar, hasMore);
			}
		}
		else {
			ppNodes = ppRoot;
	...
	if (ppNodes) {
		if (isalpha(tolower(*(str + strlen(str) - 1)))) {
			ppNodes = checkLetterForWordAndReversePrefix(ppNodes, tolower(*(str + strlen(str) - 1)), FALSE);
		}
	}
}