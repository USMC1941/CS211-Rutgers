...
*/
LetterNodes** checkLetterForWordAndPrefix(LetterNodes** ppNodes, int input, int hasMore) {
	...
	if ((*ppNodes)->letterChildIndex[index] == CHILD_PROPERTY_NOT_PRESENT) {											// Not present
		...
		if (hasMore) {																																		// Input has child
			if ((*ppNodes)->letterLeafIndex[index] < ALPHABET_COUNT) {															// This is a leaf
				...
				//
				(*((*ppNodes)->pCounters + 2 * (*ppNodes)->letterLeafIndex[index] + 1))++;								// Prefix count + 1	
			}
			else {
				// This is not a leaf. No child.
			}
		}
		...
		if (hasMore) {
			if ((*ppNodes)->letterLeafIndex[index] < ALPHABET_COUNT) {															// This is a leaf
				...
				//
				(*((*ppNodes)->pCounters + 2 * (*ppNodes)->letterLeafIndex[index] + 1))++;								// Prefix + 1
			}
			else {
				// This is not a leaf
			}
